...
\begin{itemize}
\ifnum \VersionNum = 1
\item P.22, 2.3節: $a_0 \leq a_1$ のための操作的意味論の2つ目の規則($\Tfalse$となる場合)の条件が
\begin{quote}
  $n_1$より$n_0$が大きいか等しいとき
\end{quote}
とあるのは
\begin{quote}
  $n_1$より$n_0$が大きいとき
...
\begin{itemize}
  \item P.141, 命題8.5:
  「任意の$x,y \in D$について」の前に「\new{$f$が1対1対応であって}」という条件が抜けていました．
  また，${\iff}$の右側の\old{$f(x) \sqle f(y)$}は\new{$f(x) \sqle_E f(y)$}の誤りです．
\end{itemize}
\fi
  ...
  \[
    \begin{array}{lcl}
      c \in \bar{H} &\iff& \FunCmd{c}(\Encode{c}) \diverge\\
                    ...
                    &\iff& \FunCmd{G;C}(\Encode{c})\old{)} \converge\\
    \end{array}
  \]
  は
  \[
